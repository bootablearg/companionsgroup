...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            ...
            >
                <!-- Main Photo -->
                <div class="relative bg-gray-900 rounded-lg overflow-hidden aspect-[3/4] group">
            ...
            @if($aviso->videos && $aviso->videos->count() > 0)
                <div class="mt-8">
                    <h2 class="text-lg font-semibold text-gray-200 mb-4">Videos</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach($aviso->videos->take(3) as $video)
                            <div class="bg-gray-900 border border-gray-800 rounded-lg overflow-hidden aspect-video">
            ...
            <div class="mt-8 bg-gray-900 border border-gray-800 rounded-lg p-6">
                <h2 class="text-lg font-semibold text-gray-200 mb-4">Sobre mi</h2>
            ...
            <div class="mt-6 bg-gray-900 border border-gray-800 rounded-lg p-6">
                <h2 class="text-lg font-semibold text-gray-200 mb-4">Atributos fisicos</h2>
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    ...
                    @foreach($attributes as $label => $value)
                        <div class="bg-gray-800/50 rounded-md p-3">
                            ...
                            <p class="text-sm text-gray-300 font-medium">{{ $value }}</p>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="flex flex-wrap gap-2 mt-4">
                    @if($aviso->smoker)
                        <span class="bg-gray-800 border border-gray-700 text-gray-400 text-xs px-3 py-1 rounded-full">Fumadora</span>
                    @endif
                    @if($aviso->waxed)
                        <span class="bg-gray-800 border border-gray-700 text-gray-400 text-xs px-3 py-1 rounded-full">Depilada</span>
                    @endif
                    @if($aviso->orientation)
                        <span class="bg-gray-800 border border-gray-700 text-gray-400 text-xs px-3 py-1 rounded-full">{{ ucfirst($aviso->orientation) }}</span>
                    @endif
                </div>
            </div>
            ...
            @if($aviso->languages && count($aviso->languages) > 0)
                <div class="mt-6 bg-gray-900 border border-gray-800 rounded-lg p-6">
                    <h2 class="text-lg font-semibold text-gray-200 mb-4">Idiomas</h2>
                    <div class="flex flex-wrap gap-3">
                        @foreach($aviso->languages as $lang)
                            <div class="flex items-center space-x-2 bg-gray-800 rounded-md px-3 py-2">
                                <span class="text-sm text-gray-300">{{ $lang['language'] ?? $lang }}</span>
                                @if(isset($lang['level']))
            ...
            @if($aviso->rates && $aviso->rates->count() > 0)
                <div class="mt-6 bg-gray-900 border border-gray-800 rounded-lg p-6">
                    <h2 class="text-lg font-semibold text-gray-200 mb-4">Tarifas</h2>
                        ...
                        @foreach($aviso->reviews->take(10) as $review)
                            <div class="bg-gray-900 border border-gray-800 rounded-lg p-5">
                                <div class="flex items-start justify-between mb-3">
                                    <div>
                                        <p class="text-sm font-medium text-gray-300">{{ $review->reviewer_name ?? 'Anonimo' }}</p>
            ...
            <div class="sticky top-24 space-y-4">
                ...
                <div class="bg-gray-900 border border-gray-800 rounded-lg p-6">
                    <div class="flex items-start justify-between mb-1">
                        <h1 class="text-2xl font-bold text-gray-100">{{ $aviso->display_name }}</h1>
                ...
                @if($aviso->availability)
                    <div class="bg-gray-900 border border-gray-800 rounded-lg p-5">
                        <h3 class="text-sm font-semibold text-gray-300 mb-3">Disponibilidad</h3>
                        <div class="grid grid-cols-7 gap-1 text-center">
                            ...
                            @foreach($days as $i => $day)
                                ...
                                <div class="rounded py-1.5 text-xs font-medium {{ $available ? 'bg-rose-900/40 text-rose-400 border border-rose-900/50' : 'bg-gray-800 text-gray-700' }}">
                                    {{ $day }}
                                </div>
                            @endforeach
                        </div>
