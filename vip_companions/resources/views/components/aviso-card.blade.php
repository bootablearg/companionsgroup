...
<article class="border border-border-default rounded-2xl bg-page-background shadow-xl shadow-black/40 overflow-hidden transition hover:-translate-y-0.5 hover:border-rose-500/30">
    <a href="{{ route('aviso.show', $aviso->id) }}" class="block relative h-52 w-full overflow-hidden bg-gradient-to-br from-gray-900 to-gray-800">
        ...
        <div class="absolute bottom-4 left-4 text-text-primary text-xs uppercase tracking-[0.2em]">
    ...
    <div class="p-5 space-y-3">
        <div class="flex items-start justify-between gap-3">
            <div>
                <h3 class="text-lg font-semibold text-text-primary">{{ $modelo->display_name ?? $aviso->title }}</h3>
                @if($location)
        ...
        @if($serviceNames->isNotEmpty())
            <div class="flex flex-wrap gap-2 text-xs">
                @foreach($serviceNames as $service)
                    <span class="inline-flex items-center px-3 py-1 rounded-full bg-surface-secondary/70 text-text-secondary border border-border-default">
                        {{ $service }}
                    </span>
                @endforeach
            </div>
        ...
        <div class="pt-2">
            <a href="{{ route('aviso.show', $aviso->id) }}"
               class="inline-flex items-center justify-center w-full rounded-xl border border-status-error-light-border text-rose-400 hover:bg-status-error-light-bg/10 px-4 py-2 text-sm font-semibold transition">
                Ver perfil completo
            </a>
        </div>
    </div>
